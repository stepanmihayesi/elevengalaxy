...
/**
 * Generate some star coordinates randomly
 ...
 * @param starRadius
 * @param canvasContext the canvas 2d context to be able to manipulate and draw an element inside the canvas
 */
function drawCircle(
  starx: number,
  stary: number,
 ...
 * @param canvasContext the canvas 2d context to be able to manipulate and draw element inside the canvas
 * @param {Star[]} stars - the list of stars to draw
 * @param {number} canvasWidth - the computed width of the rendered canvas HTML element
 * @param {number} canvasHeight - the computed height of the rendered canvas HTML element
 * @param canvasCenter the point { x, y, z } representing the center of the canvas
...
export function Space({
  ...
  /**
   * Draw the stars into the canvas and set the animation (id isHyperSpace = true)
   * @param {Star[]} stars - the list of stars to draw
   * @param {number} canvasWidth - the computed width of the rendered canvas HTML element
   * @param {number} canvasHeight - the computed height of the rendered canvas HTML element
   * @param canvasCenter the point { x, y, z } representing the center of the canvas
   * @param {number} focalLength - the length of the focal for the fake 3d effect
   * @param {CanvasRenderingContext2D} canvasContext - the canvas 2d context to be able to manipulate and draw element inside the canvas
   */
  const draw = (
    stars: Star[],
    canvasWidth: number,
  ...
  useEffect(() => {
    // Init / Draw / Generate stars / Animate the canvas.
